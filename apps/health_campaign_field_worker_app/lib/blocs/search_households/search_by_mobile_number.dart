...
class SearchByMobileNumberBloc extends SearchHouseholdsBloc {
  SearchByMobileNumberBloc({
    ...
    on(
      _handleSearchByMobileNumber,
      transformer: debounce<SearchHouseholdsSearchByMobileNumberEvent>(
  ...
  FutureOr<void> _handleSearchByMobileNumber(
    ...
    // Check if the search text is empty or not has full mobile number; if so, clear the results and return.
    if (event.mobileNumber.trim().isEmpty ||
        event.mobileNumber.trim().length < mobileNumberLength) {
    ...
    super.emit(state.copyWith(
      loading: true,
      searchQuery: event.mobileNumber,
    ));
    ...
    // Search for individual results using the extracted IDs and the mobile number.
